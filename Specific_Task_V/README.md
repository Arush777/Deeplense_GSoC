...
## General Future Prototype Direction

In addition to the full CaLiCo-Lens prototype, I also sketched a **simpler geometry-aware and contaminant-aware verifier direction** that can be treated as a cleaner future research path.

The motivation for this simpler prototype is practical:
- the full Stage B verifier is expressive, but also heavier,
- a simpler verifier may be easier to stabilize,
- and it may provide a better balance between interpretability, candidate purity, and implementation complexity.

This version keeps the same core idea:
- start from a strong **Stage A candidate generator**,
- reuse its morphology score and fused features,
- add a **geometry head**,
- canonicalize the input into an annular frame,
- model coarse contaminant structure from hard negatives,
- and produce a refined verification score.

The difference is that this version can be viewed as a **more modular and more directly extensible research direction**. It is useful as a bridge between the current benchmark pipeline and a future second-stage verifier that is easier to analyze, simplify, and adapt across surveys.

![CaLiCo-Lens future prototype](assets/calico-lens-future-prototype.png)

*Simpler future prototype for CaLiCo-style verification. This figure represents a cleaner research direction built on top of the benchmark candidate generator, keeping the key ideas of geometry-aware canonicalization and contaminant-aware verification while remaining easier to simplify or extend in future work.*
